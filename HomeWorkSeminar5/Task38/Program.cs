...
void MaximumElementAndTheDifferenceBetweenThem(double[] array)
{
    ...
    System.Console.WriteLine($"разница между минимальным и максимальным элементами равна : {Math.Round(Max * 1000) / 10} - {Math.Round(Min * 1000) / 10} = {Math.Round(Math.Round(Max * 1000) / 10) - (Math.Round(Min * 1000) / 10)} ");
}
